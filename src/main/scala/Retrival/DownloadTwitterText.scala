...
object DownloadTwitterText extends App{
  ...
  */
  ...
  //val tweets = TwitterClient.getFromSearchAPIByKeyword("trump", 90)
  val source  = Source.fromString(TwitterClient.getFromSearchAPIByKeyword("football", 90))
  val rts = for (t <- ingester(source).toSeq) yield t
  val rs = rts.flatMap(_.toOption)
  val tss = rs.map(r => r.statuses)
  val ts = tss.flatten
//  val arr =new ArrayBuffer[String]()
//  statuses.foreachRDD((x: RDD[String]) => arr ++ x.collect())
  val pw = new FileWriter(new File("text-tweets.txt"))
//  for(tweet <- tweets){
//    pw.append(tweet).write("\n")
//  }
  //pw.append(rs.size+"")
  print(ts.size)
  pw.flush()
